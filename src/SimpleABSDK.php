...
class SimpleABSDK {
    ...
    public function trackMetric($params) {
        ...
        if (!AggregationTypes::isValid($aggregationType)) {
            throw new \Exception("Invalid aggregation type: {$aggregationType}");
        }
        ...
        if ($aggregationType === AggregationTypes::SUM && $metricValue < 0) {
            throw new \Exception("Metric {$metricName} cannot be negative for AggregationTypes::SUM");
        }
    ...
    protected function makeApiRequest($method, $endpoint, $data = null) {
        ...
        if (!isset($http_response_header[0]) || strpos($http_response_header[0], '200') === false) {
            throw new \Exception("API request failed with status code: " . $http_response_header[0]);
        }
    ...
    private function findStage($experiment, $stage) {
        ...
        throw new \Exception("Stage {$stage} not found for experiment {$experiment['id']}");
    }
    ...
    private function findStageDimension($stageData, $dimension) {
        ...
        throw new \Exception("Dimension {$dimension} not found for stage {$stageData['stage']}");
    }
    ...
    private function findTreatment($experiment, $treatment) {
        ...
        throw new \Exception("Treatment {$treatment} not found in experiment {$experiment['id']}");
    }
